namespace ts {
    ...
    export function createLanguageService(
        ...
        function findRenameLocations(fileName: string, position: number, findInStrings: boolean, findInComments: boolean, providePrefixAndSuffixTextForRename?: boolean): RenameLocation[] | undefined {
            ...
            if (isIdentifier(node) && (isJsxOpeningElement(node.parent) || isJsxClosingElement(node.parent)) && program.getTypeChecker().getJsxImplementation(node.parent).info.isJsxIntrinsicIdentifier(node)) {
