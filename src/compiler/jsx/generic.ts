...
namespace ts {
    ...
    export function jsxi_generic(typeChecker: JsxTypeChecker, sourceFile: SourceFile): JsxImplementation {
        ...
        function getFragmentType(childrenTypes: { type: Type }[]): Type {
            return typeChecker.createArrayType(typeChecker.getUnionType(childrenTypes.map(c => c.type), UnionReduction.Literal));
        }
